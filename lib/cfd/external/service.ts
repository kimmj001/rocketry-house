...
export function createExternalCfdCacheKey(input: NormalizedExternalCfdInput) {
  ...
  return createHash("sha256").update(JSON.stringify({ geometry, mach: input.mach, angleOfAttack: input.angleOfAttack, solver: input.solver, resolution: input.resolution, version: 1 })).digest("hex").slice(0, 24);
}
